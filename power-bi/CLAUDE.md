...
│   │                           semantic-model (read TMDL), validate, references, examples, tools (all room code: pbirkit, resolve_layout, converters)
...
- **Validate at the right granularity.** Every file write is already checked automatically by the `validate-pbir.sh` hook (file-scoped, free). `pbir validate` is *whole-report only* (~2s), so run it on a **finished page**, or immediately after a hand edit / an unfamiliar visual type — not after each of ten template-driven writes. Pair it with `04-review/hooks/lint-report-traps.sh --page` (~0.15s), which catches the render-time defects schema validation never has. Cadence table: `02-build/report/validate/validate.md`.
