...
> **For remote Fabric / Service models**, prefer the Enhanced Refresh REST API. Use the canned script `scripts/refresh_model.py` — wraps every option (commit modes, parallelism, partition targeting, two-phase pattern). See `--help` for all flags.
>
> **When refresh fails**, see `refresh-troubleshooting.md` for the full debugging guide (credentials, type mismatches, timeouts, incremental refresh issues, capacity throttling, per-table isolation).
