...
| [`fix-measure-variables.cs`](fix-measure-variables.cs) | **Fix** the above — rename `VAR`s to `_`-prefixed (declaration + references) | selection → whole model | **dry-run by default**; set `apply=true` to write |
